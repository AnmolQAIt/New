...
public class DataReadWrite {
	...
	public static boolean writeDataToFile(String Property, String Data) {
		...
		try {
			...
			// String dataFolderPath = URLDecoder.decode(url.getPath(),
			// "UTF-8");
	...
	/**
	 * 
	 * This method will get the properties pulled from a file located relative
	 * to the base dir
	 * 
	 * @param propFile
	 *            complete or relative (to base dir) file location of the
	 *            properties file
	 * @param Property
	 *            property name for which value has to be fetched
	 ...
	 * @return text
	 */
	public static HashMap<String, String> readBooksFromCSVFile(String fileName) {
		...
		try {
			...
			// repeat until all lines is read
			while ((text = reader.readLine()) != null) {
				books.put(text.split("\\|")[1], text.split("\\|")[0]);
			}
		} catch (FileNotFoundException e) {
			e.printStackTrace();
		} catch (IOException e) {
